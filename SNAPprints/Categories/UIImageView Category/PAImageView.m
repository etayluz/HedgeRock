...
- (void)commonInitWithFrame:(CGRect)frame
  ...
  CGFloat radius =
      MIN(CGRectGetMidX(self.bounds) - 1, CGRectGetMidY(self.bounds) - 1);
  ...
  self.progressContainer = [[UIView alloc]
  ...
  self.containerImageView =
      [[UIImageView alloc] initWithFrame:CGRectMake(1, 1, frame.size.width - 2,
                                                    frame.size.height - 2)];
  self.containerImageView.layer.cornerRadius =
      CGRectGetWidth(self.bounds) / 2.f;
...
- (void)handleSingleTap:(id)sender {
  if (self.delegate &&
      [self.delegate respondsToSelector:@selector(paImageViewDidTapped:)]) {
...
- (void)setImageURL:(NSURL *)URL {
  ...
  UIImage *cachedImage =
      (self.cacheEnabled) ? [self.cache getImageForURL:URL] : nil;
  if (cachedImage) {
    ...
    [requestOperation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *
        ...
        if (self.cacheEnabled) {
          [self.cache setImage:image forURL:URL];
        }
    } failure:^(AFHTTPRequestOperation *operation,
                NSError *error) { NSLog(@"Image error: %@", error); }];
...
- (void)updateWithImage:(UIImage *)image animated:(BOOL)animated {
  ...
  CGFloat delay = (animated) ? 0.0 : 0.f;
  ...
  [UIView animateWithDuration:duration
      animations:^{
          self.progressContainer.transform =
          ...
          [UIView animateWithDuration:duration
                           ...
                           animations:^{
                               self.containerImageView.transform =
...
- (instancetype)init {
  ...
  if (self) {
    ...
    self.cachePath =
        [rootCachePath stringByAppendingPathComponent:spm_identifier];
...
- (void)setImage:(UIImage *)image forURL:(NSURL *)URL {
  ...
  NSString *fileExtension =
      [URL pathExtension]; //[[URL componentsSeparatedByString:@"."]
                           //lastObject];
  ...
  [imageData
      writeToFile:[self.cachePath
                      stringByAppendingPathComponent:
                          [NSString stringWithFormat:@"%lu.%@",
                                                     (unsigned long)URL.hash,
                                                     fileExtension]]
       atomically:YES];
}
...
- (UIImage *)getImageForURL:(NSURL *)URL {
  ...
  NSString *path = [self.cachePath
      stringByAppendingPathComponent:
          [NSString stringWithFormat:@"%lu.%@", (unsigned long)URL.hash,
                                     fileExtension]];
