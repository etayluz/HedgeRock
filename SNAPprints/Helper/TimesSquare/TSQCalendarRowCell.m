...
- (void)createTodayButton;
{
    ...
    self.todayButton.layer.cornerRadius = 23.0f;
...
- (void)createSelectedButton;
{
    ...
    [self.selectedButton setBackgroundColor:UIColorFromRGB(TABLE_SECTION_HEADER_BACKGROUND_COLOR)];
    ...
    self.selectedButton.layer.cornerRadius = 23.0f;
...
- (IBAction)dateButtonPressed:(id)sender;
{
    ...
    if ([selectedDate timeIntervalSinceDate:self.calendarView.minimumDate] < 0) {
        return;
    }
    
    if ([self.calendarView.maximumDate timeIntervalSinceDate:selectedDate] < 0) {
        return;
    }
